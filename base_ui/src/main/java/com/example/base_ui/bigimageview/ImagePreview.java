package com.example.base_ui.bigimageview;
...
import com.example.base_ui.bigimageview.view.ImagePreviewActivity;
import com.example.base_ui.bigimageview.view.listener.OnBigImageClickListener;
import com.example.base_ui.bigimageview.view.listener.OnBigImageLongClickListener;
import com.example.base_ui.bigimageview.view.listener.OnBigImagePageChangeListener;
import com.example.base_ui.bigimageview.view.listener.OnDownloadClickListener;
...
public class ImagePreview {
    ...
    private static final int MIN_DOUBLE_CLICK_TIME = 1500;    // 触发双击的最短时间，小于这个时间的直接返回
    ...
    public enum LoadStrategy {
        ...
        AlwaysOrigin,       // 仅加载原图；会强制隐藏查看原图按钮

        AlwaysThumb,        // 仅加载普清；会强制隐藏查看原图按钮

        NetworkAuto,        //根据网络自适应加载，WiFi原图，流量普清；会强制隐藏查看原图按钮

        Default             //手动模式：默认普清，点击按钮再加载原图；会根据原图、缩略图url是否一样来判断是否显示查看原图按钮
    }
