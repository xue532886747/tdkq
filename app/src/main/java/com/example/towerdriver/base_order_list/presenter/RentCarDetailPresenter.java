 ...
 * @date 2021/6/25
 */
public class RentCarDetailPresenter extends BasePresenter<IRentCarDetailView> {
    /**
     * 绑定view层与p层
     *
     * @param baseView
     */
    public RentCarDetailPresenter(IRentCarDetailView baseView) {
        super(baseView);
    }


    public void getRentCarDetail(String order_id, String warehouse_id, String frame_number,
...
//        }
        ...
        addToRxLife(RxHttp.request(FreeApi.api().getRentCar(params)).listener(new RxRequest.RequestListener() {
            @Override
            public void onStart() {
     ...
     * @param address
     */
    public void getRentCarDetail(String address) {
        Map<String, String> params = new HashMap<>();
        ...
        addToRxLife(RxHttp.request(FreeApi.api().getEntrepotList(params)).listener(new RxRequest.RequestListener() {
            @Override
            public void onStart() {
        ...
        }).request(new RxRequest.ResultCallback<EntrepotBean>() {
            @Override
            public void onSuccess(int code, String msg, EntrepotBean data) {
                if (isAttach()) {
                    getMvpView().onEntrepotSuccess(msg,data);
                }
            }
            ...
            public void onFailed(int code, String msg) {
                if (code == FreeApi.Code.TOKENEXPIRED) {
                    ...
                    if (isAttach()) {
                        getMvpView().onEntrepotFailure(msg);
                    }
                }
            }
        }));
