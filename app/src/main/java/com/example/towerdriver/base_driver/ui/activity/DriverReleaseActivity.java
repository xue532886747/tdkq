...
import com.example.towerdriver.base_driver.presenter.DriverReleasePresenter;
 ...
 * @date 2021/6/3
 */
public class DriverReleaseActivity extends BaseActivity<DriverReleasePresenter> implements IDriveReleaseView {
    @BindView(R.id.ll_back)
    LinearLayout ll_back;
    @BindView(R.id.et_content)
    AppCompatEditText et_content;   //内容
    @BindView(R.id.et_name)
    AppCompatEditText et_name;      //姓名
    @BindView(R.id.et_phone)
    AppCompatEditText et_phone;     //电话
    @BindView(R.id.et_title)
    AppCompatEditText et_title;     //标题
    ...
    @Override
    protected DriverReleasePresenter createPresenter() {
        return new DriverReleasePresenter(this);
    }

    @Override
    protected int getLayoutId() {
        return R.layout.act_driver_release;
    }
    ...
    private void commit() {
        ...
        if (TextUtils.isEmpty(Objects.requireNonNull(et_name.getText()).toString())) {
            ToastUtils.show("您还未编辑姓名");
            return;
        }
        name = et_name.getText().toString();
        String phone;
        if (TextUtils.isEmpty(Objects.requireNonNull(et_phone.getText()).toString())) {
            ToastUtils.show("您还未编辑电话");
            return;
        }
        phone = et_phone.getText().toString();
        String title;
        if (TextUtils.isEmpty(Objects.requireNonNull(et_title.getText()).toString())) {
        ...
        if (presenter != null) {
            ...
            presenter.release(title, content, name, phone, image);
        }
    }
     ...
     */
    @Override
    public void contentReleaseSuccess(String msg) {
        isReleaseSuccess = true;
        closeDialog();
        ToastUtils.show(msg);
        Intent intentTemp = getIntent();
        intentTemp.putExtra("isReleaseSuccess", isReleaseSuccess);
     ...
     */
    @Override
    public void contentReleaseFailure(String msg) {
        closeDialog();
        ToastUtils.show(msg);
    }
