 ...
 * @date 2021/5/21
 */
public class NewLocationManager implements ILocationCallBack, ISensorCallBack {

    private BDmap bDmap;
    private LocController locController;

    public NewLocationManager(MapView mapView,int time) {
        ...
        locController = new LocController(time);
        locController.setILocationCallBack(this);
    }
    ...
    private boolean getLocCo() {
        ...
        if (locController.getLocationClient() != null) {
            return true;
        } else {
            return false;
        }
...
//        LocController.getInstance().stop();
    ...
    public void gotoCenter(int type) {
        if (locController != null) {
            locController.setFirstLoc(true, type);
            locController.setCenter();
        }
